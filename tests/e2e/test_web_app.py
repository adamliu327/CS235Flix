...
def test_review(client, auth):
    ...
    response = client.post(
        '/review',
        data={'review': '?????', 'movie_id': 2, 'rating': 10}
    )
    assert response.headers['Location'] == 'http://localhost/movies_by_date?date=1993&view_reviews_for=2'
...
def test_review_with_invalid_input(client, auth, review, messages):
    ...
    response = client.post(
        '/review',
        data={'review': review, 'movie_id': 2, 'rating': 1}
    )
    print(response.data)
...
def test_movies_with_genre(client):
    ...
    assert b'Movies tagged by Sci-Fi' in response.data
    assert b'The Matrix' in response.data
    assert b'Gekijouban Steins;Gate: Fuka ryouiki no dejavu' in response.data